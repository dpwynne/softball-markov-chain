...
server = function(input, output) {
  ...
  X1 <- reactive(input$Number1)
  X2 <- reactive(input$Number2)
  X3 <- reactive(input$Number3)
  X4 <- reactive(input$Number4)
  X5 <- reactive(input$Number5)
  X6 <- reactive(input$Number6)
  X7 <- reactive(input$Number7)
  X8 <- reactive(input$Number8)
  X9 <- reactive(input$Number9)
  ...
  U0_nosteal[1,1] <- 1# some function of reactive(input$outs)
    # for now, assume "no steal" - runner on 1st, input$outs outs state
  U0_steal <- matrix(data = 0L, ncol = 25, nrow = 21)
  U0_steal[1,1] <- 1# function of reactive(input$success), reactive(input$outs)
  ...
  simulate_nosteal <- Simulated_inning(Softball_data1,X1,X2,X3,X4,X5,X6,X7,X8,X9)
  simuate_steal <- Simulated_inning(Softball_data1,X1,X2,X3,X4,X5,X6,X7,X8,X9)
