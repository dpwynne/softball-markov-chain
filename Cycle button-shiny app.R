...
ui <- fluidPage(sidebarPanel(width = 12,
                             useShinyjs(),
                             id = "panel",
                             # Dropdown to indicate current number of outs in the inning
                             ...
                             # Output facet graphs (steal vs. no steal) of the probability of getting each run
                             plotOutput("run_distribution"),
                             br(),
                             # Output the mean and standard deviation of both situations (steal and no steal)
                             tableOutput("result"),
),
...
server = function(input, output, session) {
  ...
  reactive({outs <- as.integer(input$outs)})
  ...
  player_transition_matrix <- function(X,Y){
    ...
    # stop command if at bats is not positive
    if(Y2$AB <= 0){
  ...
  simulate_nosteal <- reactive({
    ...
    # Simulated_inning function for "no steal" situation
  ...
  # "Steal" matrix - runner on 2nd, X outs with prob. p and no on, X+1 outs with prob. 1-p
  simulated_steal <- reactive({
    ...
    # Simulates_inning function for "steal" situation
  ...
  # summary distribution of runs scored in each situation
  ...
  observeEvent(input$goButton,{
    ...
    newSelection1 <- as.integer(input$Number2)
    updateSelectInput(session, inputId = "Number1", selected = newSelection1)
    newSelection2 <- as.integer(input$Number3)
    updateSelectInput(session, inputId = "Number2", selected = newSelection2)
    newSelection3 <- as.integer(input$Number4)
    updateSelectInput(session, inputId = "Number3", selected = newSelection3)
    newSelection4 <- as.integer(input$Number5)
    updateSelectInput(session, inputId = "Number4", selected = newSelection4)
    newSelection5 <- as.integer(input$Number6)
    updateSelectInput(session, inputId = "Number5", selected = newSelection5)
    newSelection6 <- as.integer(input$Number7)
    updateSelectInput(session, inputId = "Number6", selected = newSelection6)
    newSelection7 <- as.integer(input$Number8)
    updateSelectInput(session, inputId = "Number7", selected = newSelection7)
    newSelection8 <- as.integer(input$Number9)
    updateSelectInput(session, inputId = "Number8", selected = newSelection8)
    newSelection9 <- as.integer(input$Number1)
    updateSelectInput(session, inputId = "Number9", selected = newSelection9)
  })
